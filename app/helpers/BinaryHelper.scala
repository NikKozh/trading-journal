...
//        getPortfolioWebsocketRequest(contractDate).mapTo[TextMessage.Strict].map(_.text)
    ...
    def authorizeWebsocketRequest = {
        ...
        val webSocketFlow = Http().webSocketClientFlow(WebSocketRequest("wss://ws.binaryws.com/websockets/v3?app_id=***REMOVED***"))
...
s"""{"authorize": "9wB0Mo5iy5ZfRk3"}"""
        ...
        val (upgradeResponse, closed) =
        Http().singleWebSocketRequest(WebSocketRequest("wss://ws.binaryws.com/websockets/v3?app_id=***REMOVED***"), flow)
